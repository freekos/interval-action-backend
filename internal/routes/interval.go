...
func (r *Route) CreateInterval(c *gin.Context) {
	...
	intervalResponse := intervalToResponse(interval, []model.TaskResponse{})
...
func (r *Route) GetIntervals(c *gin.Context) {
	...
	for i, interval := range intervals {
		tasks, err := r.repository.GetTasks(interval.ID)
		if err != nil {
			errorMessage := err.Error()
			c.JSON(http.StatusInternalServerError, Response{Data: nil, Status: http.StatusInternalServerError, Error: &errorMessage})
			return
		...
		tasksResponses := make([]model.TaskResponse, len(tasks))
		for y, task := range tasks {
			tasksResponses[y] = taskToResponse(task)
		}
		intervalResponses[i] = intervalToResponse(interval, tasksResponses)
	}
...
func (r *Route) GetInterval(c *gin.Context) {
	...
	for i, task := range tasks {
		tasksResponses[i] = taskToResponse(task)
	}

	intervalResponse := intervalToResponse(interval, tasksResponses)

	c.Header("Content-Type", "application/json")
	c.JSON(http.StatusOK, Response{Data: intervalResponse, Status: http.StatusOK, Error: nil})
}

func intervalToResponse(interval model.Interval, tasks []model.TaskResponse) model.IntervalResponse {
